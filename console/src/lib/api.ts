...
export const api = {
  ...
  projects: {
    ...
    update: (
      name: string,
      data: { display_name?: string; visibility?: string },
    ) =>
    ...
    members: {
      list: (project: string) =>
        request<Member[]>(`/api/projects/${project}/members`),
      ...
      remove: (project: string, userId: string) =>
        request<{ ok: boolean }>(`/api/projects/${project}/members/${userId}`, {
          method: "DELETE",
        }),
    },
  },
  ...
  repositories: {
    list: (project: string) =>
      request<Repository[]>(`/api/projects/${project}/repositories`),
    get: (project: string, repo: string) =>
      request<Repository>(`/api/projects/${project}/repositories/${repo}`),
  },
  ...
  tags: {
    ...
    delete: (project: string, repo: string, tag: string) =>
      request<{ ok: boolean }>(
        `/api/projects/${project}/repositories/${repo}/tags/${tag}`,
        { method: "DELETE" },
      ),
  },
  ...
  robots: {
    list: (project?: string) => {
      ...
      return request<RobotToken[]>(
        `/api/robot-tokens${query ? "?" + query : ""}`,
      );
    },
    create: (data: {
      project_id: string;
      ...
      request<RobotToken>("/api/robot-tokens", {
        method: "POST",
        body: JSON.stringify(data),
      }),
    revoke: (id: string) =>
      request<{ ok: boolean }>(`/api/robot-tokens/${id}`, { method: "DELETE" }),
  },
  ...
  audit: {
    list: (limit?: number, offset?: number) =>
      request<AuditLog[]>(
        `/api/audit-logs?limit=${limit || 50}&offset=${offset || 0}`,
      ),
  },
};
